...
const FlightTable = () => {
  ...
  return (
    <div className="p-16 mb-16  mt-[72px]">
      ...
      <p className='text-[18px]  text-gray-600 font-semibold'>Choose a <span className='text-[#605DEC]'>departing</span> flight</p>
       <Link to= "/" className='text-[#605DEC] text-[17px] '><i className="fa-solid fa-arrow-left mr-2"></i>Modify Search</Link>
      </div>
      ...
      <div className="container  p-3 w-[60%] rounded-[10px] border-[1px] border-[#e1e1ec]">
        <div className="flex flex-col space-y-4">
          {flights.map((flight, index) => (
            <FlightCard key={index} flight={flight} onClick={setSelectedFlight} isSelected={selectedFlight && selectedFlight.flightCode === flight.flightCode} />
          ))}
        </div>
      </div>
